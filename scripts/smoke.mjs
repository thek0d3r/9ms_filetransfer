...
const payloads = [
  new TextEncoder().encode(`9ms individual smoke test ${new Date().toISOString()}\n`),
  new TextEncoder().encode(`9ms zip smoke test ${new Date().toISOString()}\n`),
];
...
const created = await request("/api/transfers", {
  ...
  body: JSON.stringify({
    ...
    files: [
      { name: "individual.txt", size: payloads[0].byteLength, type: "text/plain" },
      { name: "archive.txt", size: payloads[1].byteLength, type: "text/plain" },
    ],
  }),
});
...
for (const [index, remote] of session.files.entries()) {
  ...
  const put = await fetch(signed.body.urls[0].url, { method: "PUT", body: payloads[index] });
...
const lockedDownload = await fetch(`${base}/api/share/${session.shareToken}/files/${session.files[0].id}`, { redirect: "manual" });
...
const download = await fetch(`${base}/api/share/${session.shareToken}/files/${session.files[0].id}`, { headers: { Cookie: cookie }, redirect: "follow" });
if (!download.ok || Buffer.compare(Buffer.from(await download.arrayBuffer()), Buffer.from(payloads[0])) !== 0) throw new Error("Downloaded bytes did not match");
const repeatedDownload = await fetch(`${base}/api/share/${session.shareToken}/files/${session.files[0].id}`, { headers: { Cookie: cookie }, redirect: "manual" });
if (repeatedDownload.status !== 410) throw new Error(`Repeated file download should return 410, received ${repeatedDownload.status}`);
...
console.log(JSON.stringify({ ok: true, transferId: session.transferId, checks: ["multipart", "scan", "password", "one-time-download", "zip", "report", "delete", "secret-reveal-once", "secret-revoke"] }));
