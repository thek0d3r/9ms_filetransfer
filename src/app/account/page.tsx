  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-grid">
        ...
        <article className={`plan-card ${premium ? "is-premium" : ""}`}><p className="step-label">CURRENT PLAN</p><strong>{premium ? "10 GB" : "2 GB"}<small> / transfer</small></strong><p>{premium ? `${formatEuro(env.PREMIUM_PRICE_EUR_CENTS)} per month · managed securely by Stripe` : "Need to move something bigger? Premium opens the 10 GB lane."}</p>{premium ? <BillingButton portal>Manage billing</BillingButton> : env.PREMIUM_ENABLED ? <BillingButton>Upgrade to Premium →</BillingButton> : <span className="dashboard-action disabled-action">Premium coming soon</span>}</article>
