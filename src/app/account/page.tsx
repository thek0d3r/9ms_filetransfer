  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-grid">
        ...
        <article className={`plan-card ${premium ? "is-premium" : ""}`}><p className="step-label">CURRENT PLAN</p><strong>{premium ? "20 GB" : "2 GB"}<small> / transfer</small></strong><p>{premium ? account.user.subscriptionStatus === "admin_granted" ? "Premium granted by 9ms · 250 files · 30-day option" : `${formatEuro(env.PREMIUM_PRICE_EUR_CENTS)} per month · 250 files · 30-day option` : "Need to move something bigger? Premium opens the 20 GB lane."}</p>{premium ? account.user.subscriptionStatus === "admin_granted" ? <span className="dashboard-action disabled-action">Managed by 9ms</span> : <BillingButton portal>Manage billing</BillingButton> : env.PREMIUM_ENABLED ? <BillingButton>Upgrade to Premium →</BillingButton> : <span className="dashboard-action disabled-action">Premium coming soon</span>}</article>
